...
def main():
    ...
    walls.append([200, -200], [200, 200])
    walls.append([200, 200], [-200, 300])
    walls.append([-100, 200], [-300, -250])
    wall_arr = walls.get()
    ...
    while True:
        ...
        # 予測
        if count < max_frame:
            vx, vy, omega = 0, 0, 0.05  # 制御入力
        else:
            q_stdv = 0
            vx, vy, omega = 0, 0, 0
        r1, r2 = np.random.normal(0, q_stdv1, 2)
        r3 = np.random.normal(0, q_stdv2, 1)
        ...
        sim.draw_predicted_pos(robot_pos, 3*x, 3*y)
        sim.draw_finish()
        count += 1
    ...
    plt.plot(t[1:], real_x[1:], label="sim")
    plt.plot(t[1:], robot_x[1:], label="robot")
    plt.legend()
    plt.show()
